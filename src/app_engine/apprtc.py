...
#      wss_host_port_pair = constants.WSS_HOST_PORT_PAIRS[0]
  if not wss_host_port_pair:
    wss_host_port_pair = constants.WSS_HOST_PORT_PAIRS
...
def get_room_parameters(request, room_id, client_id, is_initiator):
  ...
  if len(turn_base_url) > 0:
    ...
    turn_url = 'http://104.236.11.237:8080/turnserver/' + username
...
class TurnServerPage(webapp2.RequestHandler):
  def get(self, room_id):
    expiretime = int(time.time()) + 84000
    username = room_id + ':' + str(expiretime)
    dig = hmac.new(b'logen', username, hashlib.sha1).digest()
    credential = base64.b64encode(dig)
    self.response.write(json.dumps({
      'username':username,
      'password':credential,
      'uris':['turn:104.236.11.237:3478?transport=udp',
              'turn:104.236.11.237:3478?transport=tcp',
              'turn:104.236.11.237:3479?transport=udp',
              'turn:104.236.11.237:3479?transport=tcp'
             ]
    }))
